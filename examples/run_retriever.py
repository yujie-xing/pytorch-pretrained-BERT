...
from pytorch_pretrained_bert.modeling import BertForSequenceRetrieval
...
class InputFeatures(object):
    ...
    def __init__(self, input_ids, input_mask, segment_ids, label_id):
        ...
        self.segment_ids_a, self.segment_ids_b = segment_ids
...
class RetrieverProcessor(DataProcessor):
    '''Processor for retrieval'''
    pass
...
def convert_examples_to_features(examples, label_list, max_seq_length, tokenizer):
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        if ex_index < 5:
            ...
            logger.info("tokens a: %s" % " ".join(
                [str(x) for x in tokens_a]))
            logger.info("tokens b: %s" % " ".join(
...
def main():
    ...
    # Prepare model
    model = BertForSequenceRetrieval.from_pretrained(args.bert_model,
    ...
    model = BertForSequenceRetrieval.from_pretrained(args.bert_model, state_dict=model_state_dict,
                                                          num_labels=num_labels)
    model.to(device)
